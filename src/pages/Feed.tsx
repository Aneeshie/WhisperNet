  ...
  return (
    <div className="flex flex-col h-full p-4 space-y-4">
      <header className="py-4 border-b border-zinc-900">
        ...
        <p className="text-xs font-mono text-zinc-500 mt-1">NODE_STATUS: OFFLINE_MESH | MSGS: {messages.length}</p>
      </header>
      ...
      <div className="flex-1 space-y-4 pb-4">
        {sortedMessages.length === 0 ? (
          ...
          sortedMessages.map((msg: Message) => {
            ...
            return (
              <div key={msg.id} className={`p-4 rounded-md border ${config.borderColor} ${config.bgColor}`}>
                <div className="flex justify-between items-start mb-2">
                  <div className="flex items-center space-x-2">
                    <Icon className={`w-4 h-4 ${config.textColor}`} />
                    <span className={`text-xs font-mono font-bold tracking-widest ${config.textColor}`}>
                      {config.label}
                    </span>
                  </div>
                  <div className="flex items-center text-zinc-500 space-x-1">
                    <Clock className="w-3 h-3" />
                    <span className="text-[10px] font-mono">{formatTimeAgo(msg.createdAt)}</span>
                  </div>
                </div>
                ...
                <div className="flex justify-between items-center mt-3 pt-3 border-t border-zinc-800/50">
                  <span className="text-[10px] font-mono text-zinc-600">ID: {msg.id.split('_')[1] || msg.id}</span>
